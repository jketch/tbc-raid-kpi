...
def parse_combat_log(log_path: str, allowed_bosses=None) -> dict:
    ...
    with _open_log(log_path) as f:
        for line in f:
            ...
            # ── Windfury extra-attacks RECEIVED (per player; source split in map_to_week_data) ──
            # Two event shapes, MUTUALLY EXCLUSIVE by how the player gets Windfury (so no double-count):
            #   • TOTEM (every non-shaman melee): the 2.5 client folds the totem's extra swing into the
            #     recipient's "Melee" damage (invisible in WCL), but fires SPELL_EXTRA_ATTACKS
            #     "Windfury Attack" — amount=[12] = extra attacks granted.
            #   • WEAPON (enh shamans): the Windfury Weapon imbue's swings ARE logged directly as
            #     SPELL_DAMAGE/SPELL_MISSED "Windfury Attack" (no grant event) — count each swing.
            # map_to_week_data labels the source by class (non-shaman→totem · shaman→weapon).
            if len(fields) > 10 and src_is_player and fields[10].strip('"') == "Windfury Attack":
                if ev == "SPELL_EXTRA_ATTACKS":
                    try:
                        windfury_extra[src_name] += int(fields[12])
                    except (ValueError, IndexError):
                        windfury_extra[src_name] += 1
                elif ev in ("SPELL_DAMAGE", "SPELL_MISSED"):   # a shaman's own WF Weapon swing
                    windfury_extra[src_name] += 1
