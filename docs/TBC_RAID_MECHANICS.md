...
> **Build note (backlog #2 — SHIPPED `ba8f86c`).** This table seeds `game_constants.MECHANIC_IDS`, the verified ID map behind `fetch_mechanic_compliance` (WCL `DamageTaken` per kill filtered to these IDs → per-player hits/dmg → `WEEK_DATA.mechanicCompliance`). **`mechanicCompliance` is still emitted and durable** — it's the WCL-source-of-record fallback that rebuilds `avoidableDmg` when no combat log is transferred. The **standalone "Mechanic Compliance" card was REMOVED 2026-06-13** (maintainer call — too similar to Avoidable Damage); there is no render fn for it anymore. The name-based avoidable path is **KEPT, not retired** (backlog #5c — the combat log's per-hit timestamps power the Avoidable drill-down); keep `AVOIDABLE_SPELL_NAMES` ↔ `MECHANIC_IDS` in curated parity. For **(ID unverified)** abilities, run a one-time WCL `DamageTaken`-by-ability probe (`scripts/tools/probe_mechanic_ids.py`) on a kill of that boss, then promote the live ID into this table — same discipline that built `DEBUFF_SLOTS`.
